...
```mermaid
flowchart LR
    subgraph F1["Fase 1: Hotfixes Inmediatos (24-48h)"]
        F1A["Scripts SQL de RLS & RBAC"]
        F1B["RPC process_sale_atomic Determinista"]
        F1C["Constraints CHECK Stock >= 0"]
        F1D["Parche Gemini Header Edge Function"]
    end

    subgraph F2["Fase 2: Consistencia & POS (3-5 días)"]
        F2A["Refactor Descuento Contable POS"]
        F2B["Cerrojo Síncrono POSCheckoutModal"]
        F2C["RPC execute_safe_daily_close_z"]
        F2D["Motor Matemático Determinista & COP"]
        F2E["Eliminar Polling Ciego 4s"]
    end

    subgraph F3["Fase 3: Infraestructura & Hardening (1-2 sem)"]
        F3A["Índices FK & Compuestos"]
        F3B["Cabeceras HTTP HSTS / CSP Netlify"]
        F3C["Actualización CVEs Supply Chain"]
        F3D["Hardening Vite Sourcemaps & Logs"]
    end
...
```sql
-- Función previa sin orden determinista ni kardex integrado
FOR r IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- stock_movements y cash_ledger NO se insertan aquí, quedan a expensas del frontend
END LOOP;
```
...
#### Código refactorizado listo para producción:
```sql
-- 1. Función de rol con validación de estado activo
...
-- 2. Endurecer SETTINGS (Revocar acceso anónimo)
...
-- 3. Endurecer SUPPLIERS (Solo lectura para vendedores)
...
-- 4. Endurecer PRODUCTS (Cajeros solo leen; no modifican stock ni costo directamente)
...
-- 5. Endurecer SALES y SALE_ITEMS (Vendedores solo ven sus ventas)
...
-- 6. Endurecer AUDIT_LOGS (Impedir suplantación de logs)
...
#### Código actual defectuoso (Líneas 85-93 de `saleSlice.ts`):
```typescript
...
completeSale: async (
  ...
  const { data: rpcData, error: saleError } = await supabase.rpc('process_sale_atomic', {
    ...
    p_status: status,
    p_total_usd: netTotalUSD,    // Asienta el total con descuento
  ...
  // 4. Limpieza de estado local (Kardex y Caja ya fueron asentados por la DB)
  get().clearCart();
...
performDailyClose: async (turnData) => {
    ...
    // Desfase temporal que deja ventas concurrentes en vuelo fuera del cierre Z
...
#### Código actual defectuoso (Líneas 347-349 de `POSCheckoutModal.tsx`):
```tsx
...
#### Código actual defectuoso (Líneas 87-94 de `index.ts`):
```typescript
...
#### Código actual defectuoso (Líneas 454-456 de `Dashboard.tsx`):
```typescript
