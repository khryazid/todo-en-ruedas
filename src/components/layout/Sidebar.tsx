...
import { useStore } from '../../store/useStore'; // <-- IMPORTADO EL CEREBRO
import {
...
export const Sidebar = () => {
  ...
  const { logout } = useStore(); // <-- EXTRAÍDA LA FUNCIÓN REAL
    ...
    <>
      ...
      <aside className={`fixed md:relative z-40 h-full w-64 bg-gray-900 text-white flex flex-col transition-transform duration-300 ease-in-out shadow-2xl ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-1 custom-scrollbar">
          {menuItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group font-medium ${isActive ? 'bg-red-600 text-white shadow-lg shadow-red-900/50' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
            >
        ...
        <div className="p-4 border-t border-gray-800">
          <button
            ...
            onClick={async () => {
              if (window.confirm('¿Seguro que deseas cerrar sesión?')) {
                await logout(); // <-- LLAMADA REAL A LA BASE DE DATOS
              }
            }}
          >
            <LogOut size={20} />
          ...
          <div className="mt-4 text-center"><p className="text-[10px] text-gray-600">v2.0.0 • Nube</p></div>
        </div>
      </aside>
    </>
  );
