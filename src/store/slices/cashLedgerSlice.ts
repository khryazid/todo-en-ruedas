...
const mapLedgerRow = (row: Record<string, unknown>): CashLedgerEntry => ({
  ...
  amountCOP: row.amount_cop !== null && row.amount_cop !== undefined ? Number(row.amount_cop) : undefined,
  currency: ((row.currency as 'USD' | 'BS' | 'COP') || 'USD'),
  ...
  recordCashMovement: async (entry: {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        amount_cop: entry.amountCOP ?? null,
